...
def rearrange_unzipped(dir_path, data_path, replace=False):
    ...
    if os.path.exists(img_dir) and os.path.exists(anno_dir) and not replace:
        ...
        for channel in channels:
            proc_list.append(
                Process(target=move_files, args=(
                    code_dict[channel], channel,  img_dir, org_dir))
            )
...
def img_compose(data_path: str, channels=["E", "N", "G"], mode='chw_minmax', replace=False)-> str:
    ...
    if not replace:
        if os.path.exists(output_dir):
            print(f"For image composition, using existing img_dir: {os.path.abspath(output_dir)}")
    ...
    if mode == 'chw_minmax':
        ...
        for file in tqdm.tqdm(sorted(os.listdir(os.path.join(img_dir, channels[0])))):
            org_chns = []
            for ch_id, channel in enumerate(channels):
                org_cur_chn = cv2.imread(os.path.join(
                    img_dir, channel, file), cv2.IMREAD_UNCHANGED)
...
def ann_split(ann_dir, output_dir, split_ratio=[0.8, 0.1, 0.1], replace=False):
    if not replace:
        if os.path.exists(output_dir):
            print(
                f"using existing splited ann_dir: {os.path.abspath(output_dir)}")
            return
    os.makedirs(output_dir, exist_ok=True)
    ...
    for split in split_names:
        ...
        for file in split_list:
            shutil.copy(os.path.join(img_dir, file),
                        os.path.join(split_img_dir, file))
...
if __name__ == "__main__":
    ...
    map(lambda x: shutil.rmtree(x), glob.glob(os.path.join(
        '../data', ".ipynb_checkpoints"), recursive=True))
    # unzip
    if not args.skip_unzip:
        if not os.path.exists(os.path.join(args.dst)):
            ...
            print(
                f"using existing unzipped files: {os.path.abspath(args.dst)}")
    else:
        print("skip unzip")
    ...
    # convert rgb annotation to cls_id
    ann_rgb2cls('../data', num_workers=16, replace=False)
